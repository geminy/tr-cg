...
static void stencil_reshape(int w, int h)
{
    ...
    if (w <= h) {
        ...
        gluOrtho2D(-3.0 * (GLfloat)h / (GLfloat)w, 3.0 * (GLfloat)h / (GLfloat)w, -3.0, 3.0);
    }
...
int stencil_main(int argc, char **argv)
{
    ...
    stencil_init();
    glutReshapeFunc(stencil_reshape);
    glutDisplayFunc(stencil_display);
    glutMainLoop();
