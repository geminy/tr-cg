...
/**
 * @file particle.c
 * @brief Demonstrates rendering a particle system using a vertex shader and point sprites.
 */
...
{
    int width, height;
    char *buffer = glesLoadTGA(fileName, &width, &height);
    GLuint texId;

    if (buffer == NULL) {
        glesLogMessage("[Error] Loading (%s) image.\n", fileName);
        return 0;
    }
...
void Draw(GLESContext *glesContext)
{
    ...
    glVertexAttribPointer(userData->endPositionLoc, 3, GL_FLOAT, GL_FALSE, PARTICLE_SIZE * sizeof(GLfloat),
...
int main(int argc, char *argv[])
{
    ...
    glesCreateWindow(&glesContext, "ParticleSystem", 640, 480, GLES_WINDOW_RGB);
