...
/**
 * @file multisample.c
 * @brief Shows various multi-sampling operations.
 */
...
// Initialize the stencil buffer values, and then use those values to control rendering.
void Draw(GLESContext *glesContext)
{
    ...
    // At this point, the stencil buffer will be 0x1 for all pixels.
...
void Key(GLESContext *glesContext, unsigned char key, int x, int y)
{
    switch (key) {
    case 'm':
        printf("Saw a 'm'\n");
...
int main(int argc, char *argv[])
{
    ...
    glesCreateWindow(&glesContext, "Multi-sampling", 320, 240, GLES_WINDOW_RGB);
    ...
    glesRegisterKeyFunc(&glesContext, Key);
